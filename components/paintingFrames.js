...
export const createFrames = (objects) => {
    ...
    objects.forEach((object) => {
      ...
      if (rot === 0 || rot === Math.PI) {
        // Front or Back walls
        topFrame = new THREE.BoxGeometry(size.x + extendedLength, frameThickness, frameDepth);
        bottomFrame = new THREE.BoxGeometry(size.x + extendedLength, frameThickness, frameDepth);
        leftFrame = new THREE.BoxGeometry(frameThickness, size.y + extendedLength, frameDepth);
        rightFrame = new THREE.BoxGeometry(frameThickness, size.y + extendedLength, frameDepth);
      } else if (rot === Math.PI / 2 || rot === -Math.PI / 2) {
        // Side walls
        topFrame = new THREE.BoxGeometry(size.y + extendedLength, frameThickness, frameDepth);
        bottomFrame = new THREE.BoxGeometry(size.y + extendedLength, frameThickness, frameDepth);
        leftFrame = new THREE.BoxGeometry(frameThickness, size.x + extendedLength, frameDepth);
        rightFrame = new THREE.BoxGeometry(frameThickness, size.x + extendedLength, frameDepth);
      } else {
